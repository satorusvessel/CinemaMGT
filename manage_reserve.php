...
include 'connect.php';
$ts = $con->query("SELECT * FROM seat where seat_type_ID=".$_GET['ID']);
$data = array();


while($row=$ts->fetch_assoc()){
	$data[] = $row;
	$seat_group[$row['ID']] = $row['hall_name'];
	$seat_count[$row['ID']] = $row['seat_count'];
}

 $mov = $conn->query("SELECT * FROM movie_table where ID =".$_GET['mid'])->fetch_array();
 $dur = explode('.', $mov['length_']);
